...
/*   Updated: 2023/08/15 17:41:34 by mdesmart         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	get_map_cardinal_limits(t_parsing *parsed)
{
	...
	get_map_west(parsed);
	get_map_east(parsed);
	// if ((parsed->map_south - parsed->map_north) > UINT_MAX * BOX_SIZE
	// 	|| (parsed->map_east - parsed->map_west) > UINT_MAX * BOX_SIZE)
	// 	return (ft_dprintf(2, "Error\nMap too laaaaaaarge\n"), 1);
	return (0);
}
